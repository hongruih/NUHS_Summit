...
- Lines ~47–68: `QUESTIONS` (4 open-ended questions — Q4 text uses commas, not em dashes) and `STOP_WORDS`
- Lines ~85–215: Configuration — `TURING_ITEMS` (3 Scenarios + 2 Conversations for the AI vs Human survey; each has `id`, `section`, `label`, `type`, `is_ai`, `correct_answer`; Scenarios have `patient`/`response`, Conversations have `turns[]`), `JOB_GROUPS`, `SENIORITY_LEVELS`, `TRUST_TASKS`, `ACCEPTANCE_PART_A` (8 fields: age group, gender, cluster, disciplines, years in healthcare, years in role, AI frequency, AI tools — seniority is dynamic in the template), `ACCEPTANCE_LIKERT` (22 Likert questions across Parts B–D with `[discipline]` placeholder substituted client-side)
- Lines ~220–320: Database setup (`get_db`, `init_db`) — creates 5 tables: `sentiment_responses`, `turing_responses`, `turing_answers`, `turing_tasks`, `acceptance_responses`; `turing_answers` auto-migrates from old schema (drops and recreates if `item_id` column is absent — safe pre-event, no-op once real data exists); safe `ALTER TABLE` migrations for `participant_id` on `sentiment_responses` and `cluster` + `open_reflection` on `acceptance_responses`
- Lines ~320–460: Business logic — `sentiment()` (TextBlob polarity/subjectivity), `extract_words()` (spaCy lemmatisation + NOUN/VERB/ADJ POS filter + merged stop words + `better-profanity` filter → top 80 words; falls back to regex Counter if spaCy unavailable, profanity filter applies in both paths), `get_turing_stats()` (full analytics with optional job-group filter; returns `by_item` keyed by item ID, each entry includes `section`, `label`, per-item accuracy, and per-item `avg_ratings`)
- Lines ~460+: Flask routes — participant pages, admin, all API endpoints, Excel export
...
- `survey_turing.html` — mobile AI vs Human survey (rendered at `/survey/turing`); 9-step flow: consent/intro (step 0) → demographics (step 1) → 5 items (steps 2–6: 3 Scenarios then 2 Conversations) → tasks (step 7) → results (step 8); each item has two phases — answer phase (guess + 4-dimension rating) and reveal phase (inline correct/incorrect banner, no scroll-to-top); after submitting an item the page scrolls only enough to bring the reveal banner into view; "Other" job group reveals an inline free-text input saved as `"Other; <custom text>"`; `is_ai` and `correct_answer` are embedded in the client JS for the inline reveal (security is not a concern for this booth activity)
...
| `POST /api/turing/submit` | Submit a completed AI vs Human survey (JSON: `job_group`, `seniority`, `answers[]` with `item_id`, `section`, `guess`, `ratings`, `tasks[]`) |
| `GET /api/turing/stats` | Full AI vs Human analytics (optional `?job_group=` filter); returns `by_item` with per-item accuracy + avg ratings |
| `GET /api/turing/scenarios` | Items without `is_ai`/`correct_answer` (safe for external API consumers) |
...
5. **AI vs Human** (🤖, pink) — live results: hero stats (total respondents, overall accuracy, fooled %), accuracy bars by job group and seniority, **Accuracy by Item** card showing Scenarios and Conversations in separate sub-sections each with per-item accuracy bar and inline Trust/Empathy/Safety/Usefulness averages, trusted AI tasks chart; filterable by job group
6. **AI Perspectives** (💬, cyan) — all **4** questions' word clouds and sentiment distributions side-by-side
...
- `turing_answers`: one row per item per respondent — columns: `item_id` (e.g. `scenario_1`, `conversation_2`), `section` (`Scenario` or `Conversation`), `guess` (`Human Clinician` or `AI`), `correct` (0/1), four ratings 1–5; **schema was fully replaced** from the old `guessed_ai_index`/`scenario_id` format — auto-migration drops and recreates the table if `item_id` column is absent (safe pre-event only)
...
- `is_ai` and `correct_answer` fields in `TURING_ITEMS` are **never stored in the DB** — they are embedded in the survey page JS for client-side inline reveal only
...
| 34 | AI vs Human survey fully redesigned: replaced 5 pick-the-AI scenarios with 3 Scenarios + 2 Conversations (each with a single response/dialog to judge); added consent/intro screen (step 0); per-item inline reveal after guess + ratings (no submit-then-results-page flow); `turing_answers` DB schema replaced (`item_id`, `section`, `guess` replacing `scenario_id`, `guessed_ai_index`); `get_turing_stats()` returns `by_item` with section grouping and per-item avg ratings; admin "Accuracy by Item" card shows Scenarios + Conversations sub-sections with per-item ratings inline; Excel Sheet 2 updated with new item labels | ✅ Done |
| 35 | AI vs Human survey scroll fix: removed `window.scrollTo(0,0)` from `submitItem()`; replaced with `scrollIntoView({behavior:'smooth',block:'nearest'})` on the reveal banner so participants see the correct/incorrect result without scrolling | ✅ Done |
| 36 | Admin dashboard Live Overview + AI Perspectives tab: fixed missing Q4 word cloud — `QN=3` was hardcoded; changed `loadOv()` and `loadDash()` loops to iterate `QS.length` (4); added Q4 static HTML card (`ovWC3`); added `.q3` CSS colour classes (green) for word cloud cards, per-question stat blocks, and AI Perspectives blocks; updated grid columns from `repeat(3,1fr)` to `repeat(4,1fr)`; `lastWCData` initialised dynamically to `QS.length` slots | ✅ Done |
| 37 | Q4 text: replaced em dashes with commas — "How do you think AI could best support, rather than replace, the human elements of healthcare?" (single source in `QUESTIONS`, propagates to all display locations) | ✅ Done |
